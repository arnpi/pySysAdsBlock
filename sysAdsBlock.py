...
class Sysadsblock():
    ...
    def detect_os(self):
        ...
        if settings.DEBUG:
            print "detect_os()"
        system_env = dict()
        system_env["sys"] = platform.system()
        system_env["release"] = platform.release()
        system_env["os"] = os.name
        system_env["arch"] = platform.architecture()[0]
        for el in system_env:
            if settings.DEBUG:
                print el + " = " + system_env[el]
        return system_env

    def configuration(self, system_env):
        """
        Generate personnal config with system_env from detect_os
        Return array
        """
        if settings.DEBUG:
            print "config()"
    ...
    def build_app_hosts(self, hostsPage):
        ...
        hostsFile.write("# ----------------------------------------------------------\n\n")
        hostsFile.write(
            "# This hosts file was generated by pyDenyAll from " + settings.PROVIDER[settings.DEFAULT_PROVIDER] + "\n")
